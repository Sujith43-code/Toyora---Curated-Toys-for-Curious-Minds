...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Navigation route parsing based on window location hash
  const parseHashRoute = (rawHash: string): ViewRoute => {
    try {
      const cleanHash = decodeURIComponent(rawHash.replace(/^#\/?/, '').split('?')[0]);
      if (!cleanHash) return { type: 'home' };
      if (cleanHash === 'shop') return { type: 'shop' };
      ...
      if (cleanHash === 'playrooms') return { type: 'playrooms' };
      if (cleanHash === 'gift-finder') return { type: 'gift-finder' };
      if (cleanHash === 'admin') return { type: 'admin' };
      if (cleanHash.startsWith('order-success/')) {
        const orderId = cleanHash.split('/')[1];
        return orderId ? { type: 'order-success', orderId } : { type: 'home' };
      }
      return { type: 'home' };
    } catch {
      return { type: 'home' };
  ...
  useEffect(() => {
    const handleHashChange = () => {
      setCurrentRoute(parseHashRoute(window.location.hash));
    };
  ...
  // Admin Product Edits via backend API
